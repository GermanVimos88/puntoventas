...
class ProductoPopup(Popup):
    ...
    def abrir(self, agregar, producto=None):
        if agregar:
            self.ids.producto_info_1.text='Agregar prducto nuevo'
            self.ids.producto_codigo.disabled=False
        else:
            self.ids.producto_info_1.text='Modificar prducto'
...
class VistaProductos(Screen):
    ...
    #def agregar_producto(self, agregar=False):
        if agregar: 
...
class VistaVentas(Screen):
	...
	def crear_csv(self):
		...
		if self.ids.ventas_rv.data:
			...
			self.ids.notificacion.text='CSV creada y guardada'
		else:
			self.ids.notificacion.text='No hay datos para guardar'
